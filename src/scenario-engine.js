  ...
  runRisingTemp() {
    ...
    const escalate = () => {
      ...
      if (DAO.snapshot().CORE_TEMP.v > 1160 && !S.alarms.find(a=>a.id==='DEMO-A03')) {
        ...
        addAIMessage('🚨 P1 CRITICAL ALARM: Core temperature at ' + DAO.snapshot().CORE_TEMP.v.toFixed(1) + '°C — 97% of trip limit. SCRAM RECOMMENDED IMMEDIATELY. Predicted trip in T+90s without action.');
        showDemoBar('🚨 P1 CRITICAL — Core temp ' + DAO.snapshot().CORE_TEMP.v.toFixed(0) + '°C — SCRAM REQUIRED', '#e31a1a');
      ...
      if (DAO.snapshot().CORE_TEMP.v > 1195 && !S.scramActive) {
        ...
        addAIMessage('🚨 AUTOMATIC SCRAM EXECUTED by Reactor Protection System. Core temp: ' + DAO.snapshot().CORE_TEMP.v.toFixed(1) + '°C exceeded 1195°C setpoint. All control rods inserting. Shutdown initiated.');
  ...
  runLOCA() {
    ...
    const escalate = () => {
      ...
      if (step === 6) {
        ...
        addAIMessage('🚨 CORE TEMP: ' + DAO.snapshot().CORE_TEMP.v.toFixed(1) + '°C · Rate: +' + (7+step*.6).toFixed(0) + '°C/min. Pump-B overloaded at 118% rated. Risk of second pump failure. SCRAM AND DEPRESSURIZE NOW.');
        showDemoBar('🚨 P1 LOCA — Core temp ' + DAO.snapshot().CORE_TEMP.v.toFixed(0) + '°C rising — SCRAM REQUIRED', '#e31a1a');
      ...
      if (DAO.snapshot().CORE_TEMP.v > 1180 && !S.scramActive) {
        ...
        addAIMessage('🚨 REACTOR PROTECTION SYSTEM ACTUATED. Core temp ' + DAO.snapshot().CORE_TEMP.v.toFixed(1) + '°C, primary flow < 40% nominal. All control rods inserting. Passive lead cooling: ACTIVATED.');
  ...
  runBlackout() {
    ...
    const degrade = () => {
      ...
      if (step === 6) {
        ...
        addAIMessage('🚨 EDG-2 FAILED. SCRAM bus voltage: ' + DAO.snapshot().SCRAM_V.v.toFixed(1) + 'V — approaching undervoltage trip. Battery reserve: ~4 minutes. PASSIVE SCRAM ACTUATING.');
      }
